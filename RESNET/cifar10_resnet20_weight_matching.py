...
from PWM import procrustes_weight_matching, apply_rotation
...
from plot import plot_interp_loss, plot_interp_acc
...
if __name__ == "__main__":
  ...
  with wandb.init(
    ...
    model_b = load_model(
        Path(wandb_run.use_artifact(
            f"skainswo/git-re-basin/cifar10-resnet-weights:{config.model_b}")
            .get_entry(filename).download()))
    ...
    train_loss_interp_PWM = []
    test_loss_interp_PWM = []
    train_acc_interp_PWM = []
    test_acc_interp_PWM = []

    # trova rotazioni Q che allineano B ad A
    Qs = procrustes_weight_matching(random.PRNGKey(config.seed), permutation_spec, flatten_params(model_a), flatten_params(model_b), max_iter=10)
    # applica le rotazioni a params_b
    params_b_aligned = unflatten_params(apply_rotation(permutation_spec, Qs, flatten_params(model_b)))

    # PWM interpolation
    for lam in tqdm(lambdas):
      clever_p = lerp(lam, unfreeze(model_a), unfreeze(params_b_aligned))
      ...
      train_loss_interp_PWM.append(train_loss)
      test_loss_interp_PWM.append(test_loss)
      train_acc_interp_PWM.append(train_acc)
      test_acc_interp_PWM.append(test_acc)
    ...
    for lam in tqdm(lambdas):
      clever_p = lerp(lam, unfreeze(model_a), unfreeze(model_b_clever))
      train_loss, train_acc = stuff["dataset_loss_and_accuracy"](clever_p, train_ds, 1000)
      test_loss, test_acc = stuff["dataset_loss_and_accuracy"](clever_p, test_ds, 1000)
    ...
    fig = plot_interp_loss(config.load_epoch, lambdas, train_loss_interp_naive,
                           ...
                           test_loss_interp_clever, train_loss_interp_PWM,
                           test_loss_interp_PWM)
    ...
    fig = plot_interp_acc(config.load_epoch, lambdas, train_acc_interp_naive, test_acc_interp_naive,
                          train_acc_interp_clever, test_acc_interp_clever, train_acc_interp_PWM,
                          test_acc_interp_PWM)
